...
class BPNN_BGD():
    ...
    def update_parameters(self):
        ...
        '''
        for i in range(len(self.weights)):
            self.weights[i] -= self.learning_rate * self.partials[i]

            self.bias[i] -= self.learning_rate * self.partials_b[i]
    ...
    def fit(self, train_data, train_label, learning_rate=0.1, epochs=10):
        ...
        for epoch in pbar:
            self.partials = []
            self.partials_b = []
            self.forward_propagation(train_data[0])
            ...
            for i in range(1, len(train_data)):
                ...
                self.back_propagation(train_label[i])
                for j in range(len(self.partials)):
                    self.partials[j] += np.dot(self.deltas[j], self.layers_out[j]).T
                    self.partials_b[j] += self.deltas[j].T
            for i in range(len(self.partials)):
                self.partials[i] /= len(train_data)
                self.partials_b[i] /= len(train_data)
            self.update_parameters()
        #     outputs = []
        #     i = np.random.randint(train_data.shape[0])
        #     outputs.append(self.forward_propagation(train_data[i]))
